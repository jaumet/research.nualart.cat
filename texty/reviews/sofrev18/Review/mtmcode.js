...
function MTMBrowser() {
	...
	this.preHREF = location.href.substring(0, location.href.lastIndexOf("http://informationr.net/") +1)
}
...
function MTMresolveURL(thisURL, testLocal) {
	...
	if(!tempString) {
		if(thisURL.indexOf("http://informationr.net/") == 0) {
			tempString = location.protocol + "//" + location.hostname + thisURL;
		} else if(thisURL.indexOf("../index.html") == 0) {
			tempString = this.preHREF;
			do {
				thisURL = thisURL.substr(3);
				tempString = tempString.substr(0, tempString.lastIndexOf("http://informationr.net/", tempString.length-2) +1);
			} while(thisURL.indexOf("../index.html") == 0);
			tempString += thisURL;
		} else {
...
var MTMCodeFrame = "code", MTMenuFrame = "menu", MTMTableWidth = "100%", MTMenuImageDirectory = "menu-images/index.html", MTMenuBackgroundDirectory = "";
